...
const play = {
  ...
  async execute(interaction, client) {
    ...
    if (!vc) return interaction.editReply({ content: 'Join a voice channel first!' });
...
async function _play(ctx, query, vc, client, isPrefix = false) {
  try {
    ...
    const requester = isPrefix ? ctx.author?.tag : ctx.user?.tag;
    ...
    if (isFirst) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(songs.length === 1 ? '[' + songs[0].title + '](' + songs[0].url + ')' : songs.slice(0, 5).map((s, i) => (i+1) + '. ' + s.title).join('\n'))
        .addFields(
      ...
      if (isPrefix) await ctx.edit({ content: '', embeds: [embed] }).catch(() => ctx.channel.send({ embeds: [embed] }));
      else await ctx.editReply({ embeds: [embed] }).catch(() => {});
    }
  } catch (err) {
    console.error(err);
    const errMsg = 'Error: ' + (err.message || 'Could not find or play that song.');
...
const skip = {
  ...
  async execute(interaction, client) {
    ...
    queue.player.stop();
    interaction.reply({ content: 'Skipped ' + skipped, ephemeral: true });
  },
...
const stop = {
  ...
  async execute(interaction, client) {
    ...
    interaction.reply({ content: 'Stopped and cleared the queue.', ephemeral: true });
  },
  async executePrefix(message, _, client) {
    const queue = client.queues.get(message.guild.id);
...
const loop = {
  ...
  async execute(interaction, client) {
    ...
    queue.loop = mode;
    const labels = { song: 'Song loop on', queue: 'Queue loop on', none: 'Loop off' };
  ...
  async executePrefix(message, args, client) {
    ...
    const mode = modes[args[0]?.toLowerCase()] ?? (queue.loop === 'none' ? 'song' : queue.loop === 'song' ? 'queue' : 'none');
    queue.loop = mode;
    const labels = { song: 'Song loop on', queue: 'Queue loop on', none: 'Loop off' };
...
const shuffle = {
  ...
  async execute(interaction, client) {
    ...
    interaction.reply({ content: queue.shuffle ? 'Shuffle on' : 'Shuffle off', ephemeral: true });
  },
  async executePrefix(message, _, client) {
    const queue = client.queues.get(message.guild.id);
    if (!queue) return message.reply('Nothing is playing.');
    queue.shuffle = !queue.shuffle;
    message.reply(queue.shuffle ? 'Shuffle on' : 'Shuffle off');
  },
};
...
function buildQueueEmbed(q) {
  const list = q.songs.slice(0, 15).map((s, i) => (i+1) + '. ' + s.title + ' (' + (s.duration || '?') + ')').join('\n');
  return new EmbedBuilder()
    .setColor(0x5865F2)
    .setTitle('Queue')
    .addFields(
      { name: 'Now Playing', value: q.currentSong.title, inline: false },
    ...
    .setFooter({ text: q.songs.length + ' song(s) in queue' });
}
...
const np = {
  aliases: ['nowplaying'],
  slash: new SlashCommandBuilder().setName('np').setDescription('Show what is currently playing').toJSON(),
  async execute(interaction, client) {
    const q = client.queues.get(interaction.guild.id);
...
function buildHelpEmbed() {
  return new EmbedBuilder()
    ...
    .addFields(
      { name: 'Playback', value: '/play /pause /resume /skip /stop', inline: false },
      { name: 'Queue', value: '/queue /loop /shuffle', inline: false },
      { name: 'Settings', value: '/volume /np /leave', inline: false },
      { name: 'Prefix', value: 'All commands work with ! prefix too', inline: false },
      { name: 'Loop modes', value: '/loop song - repeat current\n/loop queue - repeat all\n/loop off - no loop', inline: false },
    )
    .setFooter({ text: 'SENSEQUALITY Music Bot' });
}
