...
async function resolveSong(query) {
  ...
  if (ytdl.validateURL(query)) {
    ...
    return [{
      url: query, title: d.title, duration: formatDuration(d.lengthSeconds),
      thumbnail: d.thumbnails.pop()?.url, requester: null, source: 'youtube',
    }];
  }
...
async function searchYouTube(query) {
  ...
  return [{ url: v.url, title: v.title, duration: v.timestamp, thumbnail: v.thumbnail, requester: null, source: 'youtube' }];
}

async function createStream(song) {
  const attempts = [
    { label: 'highestaudio', fn: () => ytdl(song.url, { filter: 'audioonly', quality: 'highestaudio', highWaterMark: 1 << 25 }) },
    { label: 'lowestaudio',  fn: () => ytdl(song.url, { filter: 'audioonly', quality: 'lowestaudio',  highWaterMark: 1 << 25 }) },
    { label: 'opus filter',  fn: () => ytdl(song.url, { filter: f => f.codecs === 'opus' && f.container === 'webm', highWaterMark: 1 << 25 }) },
    { label: 're-search', fn: async () => {
